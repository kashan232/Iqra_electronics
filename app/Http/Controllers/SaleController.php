...
class SaleController extends Controller
{
    ...
    public function store_Sale(Request $request)
    {
        ...
        $changeToReturn = (float)$request->input('cash_return', 0);
        ...
        $taxPercentage = (float) $request->input('tax', 0);
        $totalPrice = (float) $request->input('total_price', 0);

        $taxAmount = ($totalPrice * $taxPercentage) / 100; // Calculate tax as percentage
        $netTotal = $totalPrice + $taxAmount; // Correct calculation
        ...
        $saleData = [
            ...
            'total_price' => $totalPrice,
            'tax' => $taxPercentage, // Store percentage only
            'Payable_amount' => $netTotal, // Corrected value
            'cash_received' => $cashReceived,
